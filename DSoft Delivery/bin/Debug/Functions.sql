...
CREATE OR REPLACE FUNCTION dsoft_fecha_dia(
    p_usuario integer,
    p_data date)
  RETURNS integer AS
$BODY$
...
begin
	...
	n_saida_total := n_saida + n_vale + n_despesa + n_pagamento;
...
-- Function: dsoft_fecha_caixa_saida(integer, double precision, integer)

-- DROP FUNCTION dsoft_fecha_caixa_saida(integer, double precision, integer);

CREATE OR REPLACE FUNCTION dsoft_fecha_caixa_saida(
    p_caixa integer,
...
declare v_entrada		double precision;
declare v_saida			double precision;
declare v_saida_fechamento      double precision;
declare v_despesa		double precision;
declare v_vale			double precision;
declare v_pagamento		double precision;
declare v_transferencia		double precision;
...
begin
	...
	if (v_saldo > p_saldo) then
		v_saida_fechamento := (v_saldo - p_saldo);
		v_saida := v_saida + v_saida_fechamento;
		v_saldo := p_saldo;

		insert into caixa_fluxo (tipo, caixa, usuario, valor, observacao) values ('S', p_caixa, p_usuario, v_saida_fechamento, 'FECHAMENTO DE CAIXA');
	end if;
